...
if [[ $# -lt 1 ]] || [[ "$1" == "--"* ]]; then
  ...
  exec java -Duser.home="$JENKINS_HOME" "${java_opts_array[@]}" -jar ${JENKINS_WAR} JENKINS_JAVA_OPTS="-Dhttps.protocols=TLSv1.2 -Djdk.tls.client.protocols=TLSv1.2" "${jenkins_opts_array[@]}" "$@"
fi
