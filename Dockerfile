...
RUN echo -e "jenkins:x:1000:" >> /etc/group && \
    ...
    microdnf -y update && microdnf -y install java-11-openjdk wget unzip tar git && \
    mkdir -p /usr/share/jenkins/ref /usr/share/jenkins/ref/plugins /var/jenkins_home && \
    wget -qqO /usr/share/jenkins/jenkins.war https://get.jenkins.io/war/2.384/jenkins.war && \
